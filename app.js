    ...
    displayCurrentConcentrations(bodyWeight, age) {
        ...
        this.doses.forEach(doseEntry => {
            ...
            if (currentConc > 0.01) { // Only show if concentration > 0.01 ng/mL
                const normalized = DrugNormalization.normalizeToAlcoholEquivalent(currentConc, doseEntry.drug);
        ...
        const impairmentLevel = DrugNormalization.getImpairmentLevel(totalNormalized);
        summaryHTML += `
            <div style="margin-top: 15px; padding: 15px; background: #e9ecef; border-radius: 4px;">
                <strong>Combined Impairment Level: ${impairmentLevel}</strong><br>
                <small>Alcohol-equivalent concentration: ${totalNormalized.toFixed(1)}</small>
            </div>
        `;
        
        if (totalNormalized < 0.1) {
